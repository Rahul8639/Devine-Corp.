...
export default function EditFeaturedProductPage() {
  ...
  const [title, setTitle] = useState('Mouse & Keyboards');
  const [products, setProducts] = useState<FP[]>([]);
  const [form, setForm] = useState<FP>({ id: Date.now(), name: '', price: '₹2550/-', mrp: '5000', rating: '4.2', image: '', brand: 'Divine' });
  ...
  const save = (next: FP[]) => {
    setProducts(next);
    try { localStorage.setItem('featuredProductsData', JSON.stringify(next)); } catch {}
  };
  const saveTitle = (val: string) => {
    setTitle(val);
    try { localStorage.setItem('featuredTitle', val); } catch {}
  };

  const add = () => {
    if (!form.name.trim()) return;
    save([...products, { ...form, id: Date.now() }]);
    setForm({ id: Date.now(), name: '', price: '₹2550/-', mrp: '5000', rating: '4.2', image: '', brand: 'Divine' });
  };
  const remove = (idx: number) => save(products.filter((_, i) => i !== idx));
    ...
    <div>
      ...
      <div style={{ maxWidth: 1100, margin: '0 auto', padding: '1.5rem' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1rem' }}>
          <h1 style={{ fontWeight: 500 }}>Edit Featured Product</h1>
          <motion.button 
        ...
        <div className="management-section">
          ...
          <div className="input-group" style={{ flexDirection: 'column', alignItems: 'stretch', gap: '0.5rem' }}>
            <label>Title</label>
            <input className="image-url-input" value={title} onChange={(e) => saveTitle(e.target.value)} />
          </div>
        </div>
        ...
        <div className="management-section">
          ...
          <div style={{ marginTop: '0.75rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
            <input type="file" accept="image/*" onChange={(e) => { const f = e.target.files && e.target.files[0]; if (f) onUploadFile(f); if (e.target) e.target.value = '' as any; }} />
            {uploading && <span style={{ fontSize: 14, opacity: 0.8 }}>Uploading...</span>}
            {uploadError && <span style={{ fontSize: 14, color: '#f87171' }}>{uploadError}</span>}
          </div>
        </div>
          ...
          {products.map((p, idx) => (
            <div key={p.id} className="image-item" style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', padding: '0.5rem 0' }}>
              ...
              <div className="image-actions" style={{ display: 'flex', gap: 4 }}>
                ...
                <button onClick={() => move(idx, idx + 1)} disabled={idx === products.length - 1} className="move-button down">↓</button>
